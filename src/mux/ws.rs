...
pub trait WebSocket
where
    Self: Stream<Item = Result<Self::Message, <Self as WebSocket>::Error>>
        + Sink<Self::Message, Error = <Self as WebSocket>::Error>
        + Send
        + Unpin
        + 'static,
    <Self as Sink<Self::Message>>::Error: std::fmt::Debug,
{
    ...
    /// Specialized for our [`Message`] type. See [`futures_util::Sink::poll_ready`] for more details.
